...
export const loginSchema = z.object({
  email: z
    .string({
      required_error: "ইমেইল বা ফোন নম্বর প্রয়োজন",
    })
    .toLowerCase()
    .trim()
    .refine((val) => {
      // Allow either valid email OR valid phone number
      const isEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val);
      const isPhone = /^01[0-9]{9}$/.test(val);
      return isEmail || isPhone;
    }, "অনুগ্রহ করে একটি সঠিক ইমেইল বা ফোন নম্বর দিন"),
