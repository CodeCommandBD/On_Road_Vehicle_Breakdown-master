...
export class ValidationError extends AppError {
  constructor(message = MESSAGES.ERROR.VALIDATION_FAILED, errors = []) {
...
export class ForbiddenError extends AppError {
  constructor(message = MESSAGES.ERROR.FORBIDDEN) {
...
export class NotFoundError extends AppError {
  constructor(message = "রিসোর্স খুঁজে পাওয়া যায়নি") {
...
export class ConflictError extends AppError {
  constructor(message = "ডেটা ইতিমধ্যে বিদ্যমান") {
...
function handleDuplicateKeyError(error) {
  ...
  let message = `${field} "${value}" ইতিমধ্যে ব্যবহৃত হয়েছে`;
...
export function handleError(error) {
  ...
  if (error.name === "CastError") {
    return NextResponse.json(
      {
        success: false,
        message: `অবৈধ ${error.path}: ${error.value}`,
        timestamp: new Date().toISOString(),
      },
      { status: HTTP_STATUS.BAD_REQUEST }
    );
